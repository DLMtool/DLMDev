...
# visualize and report the resulting management advice, and explore the sensitivity
# of the management advice to the import data. 
...
# total allowable catches (which may be interpreted as the overfishing 
# limit, OFL). These are MPs of class 'Output' and can be displayed using:
...
# 'Fratio' is included in the output of the 'Can' function so we know this method 
# can be applied to our data object. 
...
#
# All Output MPs provide stochastic estimates of the TAC and therefore
...
# position in the `Data` object that refers to the data corresponding to that 
# particular iteration. 
# In the MSE the Fishery Data object is populated with data that is generated by
# the simulation model.  During MSE the value of `x` goes from 1 to the total 
# number of simulations (`nsim`). This means that it is easy to apply the same 
# MP function simultaneously to many different simulated data sets.
#
...
# The 'TAC' slot now contains 100 samples (reps=100 in call to TAC above) of the 
# recommended TAC  for each of the MPs that we ran:
...
# The purpose of this slot is to set the current level of fishing effort.
...
Atlantic_mackerel <- Sense(Atlantic_mackerel, "DBSRA4010")

# Q4.1  What data is DBSRA4010 most sensitive to? Given what you know about 
#       DBSRA4010 method (see the help file for details), does this result make 
#       sense?



Atlantic_mackerel <- Sense(Atlantic_mackerel, "MCD")

Atlantic_mackerel <- Sense(Atlantic_mackerel, "MCD4010")

# Q4.2  How do these two methods differ? Is this difference visible in the 
#       sensitivity plot?
