...
MyObs = Generic_Obs
  ...
  MyObs@ Mbiascv       = c(0.2)            # guess
  MyObs@ Kbiascv       = c(0.1)            # guess
  MyObs@ t0biascv      = c(0.1)            # guess
  MyObs@ Linfbiascv    = c(0.05)           # guess
  MyObs@ LFCbiascv     = c(0.05)           # guess
  MyObs@ LFSbiascv     = c(0.05)           # guess
  # MyObs@ B0biascv      = c(3)              # guess
  MyObs@ FMSYbiascv    = c(0.2)            # guess
  MyObs@ FMSY_Mbiascv  = c(0.2)            # guess
  MyObs@ BMSY_B0biascv = c(0.2)            # guess
  # MyObs@ rcbiasv       = c(0.5)            # guess
  MyObs@ LenMbiascv    = c(0.1)            # guess
  MyObs@ Dbiascv   = c(0.5)            # guess
  MyObs@ Dobs      = c(0.05, 0.1)      # guess
  MyObs@ Btbiascv  = c(0.333, 3)       # guess
  MyObs@ Btobs     = c(0.2, 0.5)       # guess
  # MyObs@ Fcurbiascv= c(0.5)            # guess
  # MyObs@ Fcurcv    = c(0.5, 1)         # guess
  MyObs@ hbiascv       = c(0.2)            # guess
  MyObs@ Ibiascv   = c(0.2)            # guess
  # MyObs@ maxagecv  = c(0.1)            # guess
  MyObs@ beta      = c(0.5, 2)         # guess
  MyObs@ Recbiascv     = c(0.1, 0.3)       # guess
  MyObs@ Irefbiascv    = c(0.2)            # guess
  MyObs@ Crefbiascv    = c(0.2)            # guess
  MyObs@ Brefbiascv    = c(0.5)            # guess
